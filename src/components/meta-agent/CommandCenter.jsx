...
import { Search, FileText, Package, Shield, Wrench, ArrowUp, CheckSquare, Lock, Globe, Bot, Workflow, CheckCircle2, Gauge, Loader2 } from "lucide-react";
        ...
        {COMMANDS.map((c) => {
          ...
          return (
            <button
              ...
              className="flex flex-col items-start gap-1 p-3 rounded-xl border border-stone-200 bg-stone-50 hover:border-amber-400 hover:bg-amber-50 transition disabled:opacity-40 disabled:cursor-not-allowed text-left group"
            >
              <div className="flex items-center gap-2">
                {running === c.cmd ? (
                  <Loader2 className="h-4 w-4 text-amber-500 animate-spin" />
                ) : (
                  <Icon className="h-4 w-4 text-stone-500 group-hover:text-amber-600" />
                )}
                <span className="text-xs font-bold text-stone-700 group-hover:text-amber-700">/{c.cmd}</span>
              </div>
              <p className="text-xs text-stone-400 leading-tight">{c.desc}</p>
            </button>
          );
        })}
      </div>
