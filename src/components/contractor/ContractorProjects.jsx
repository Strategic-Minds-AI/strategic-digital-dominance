      ...
      <div>
        ...
        <div className="space-y-3">
          ...
          {active.map((p) => {
            ...
            return (
              <div key={p.id} className="xa-card xa-card-hover">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <div className="text-sm font-bold text-white">{p.client_name}</div>
                ...
                {p.flake_color_hex && (
                  <div className="flex items-center gap-2 mt-2">
                    <span className="w-4 h-4 rounded-full border border-stone-600" style={{ background: p.flake_color_hex }} />
      ...
      {completed.length > 0 && (
        <div>
          ...
          <div className="space-y-2">
            {completed.map((p) => (
              <div key={p.id} className="xa-card flex items-center gap-3">
                <CheckCircle2 className="h-5 w-5 text-green-400 shrink-0" />
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-semibold text-white truncate">{p.client_name}</div>
