...
// ===== APP-SPECIFIC =====
// Syncs the 60-Day Contractor Workflow into the Swarm as trackable SwarmTask records.
// Alpha Prime (the orchestrator) uses these tasks for persistent self-reflection tracking
// and autonomous implementation. Each workflow day becomes a SwarmTask that Alpha Prime
// can monitor, update status on, and mark as implemented when the corresponding tool
// goes live. Also creates an Investigation record for Alpha Prime's persistent awareness.
//
// This function is admin-only — verifies user.role === 'admin' before proceeding.
...
export default async function(req) {
  try {
    ...
    // Step 1: Check which workflow tasks already exist (avoid duplicates)
    const existingTasks = await base44.asServiceRole.entities.SwarmTask.filter({
      task_type: "workflow_implementation",
    });

    const existingDays = new Set(
      (existingTasks || []).map((t) => t.payload?.params?.workflowDay).filter(Boolean)
    );

    // Step 2: Create SwarmTask records for workflow days not yet tracked
    const tasksToCreate = [];
    for (const day of WORKFLOW_DAYS) {
      if (existingDays.has(day.day)) continue;
      const stage = WORKFLOW_STAGES.find((s) => s.id === day.stage);
      tasksToCreate.push({
        task_type: "workflow_implementation",
        title: `Day ${day.day}: ${day.tool}`,
        description: `Stage ${day.stage} — ${stage?.name}. WITHOUT AI: ${day.withoutAi} | WITH AI: ${day.withAi} | Tool route: ${day.toolRoute} | Retail: ${day.retailPrice} → Delivered: ${day.deliveredCost} (Save ${day.savings})`,
        priority: day.day <= 10 ? "critical" : day.day <= 30 ? "high" : "normal",
        status: "pending",
        assigned_agent: "alpha_prime_orchestrator",
        created_by_agent: "system",
        payload: {
          action: "implement_workflow_tool",
          entity_type: "workflow_day",
          params: {
            workflowDay: day.day,
            stage: day.stage,
            stageName: stage?.name,
            tool: day.tool,
            toolRoute: day.toolRoute,
            retailPrice: day.retailPrice,
            deliveredCost: day.deliveredCost,
            savings: day.savings,
            withoutAi: day.withoutAi,
            withAi: day.withAi,
          },
        },
      });
    ...
    if (tasksToCreate.length > 0) {
      const created = await base44.asServiceRole.entities.SwarmTask.bulkCreate(tasksToCreate);
      createdCount = created?.length || 0;
    }

    // Step 3: Create or update Alpha Prime's persistent Investigation (self-reflection tracker)
    const existingInvestigations = await base44.asServiceRole.entities.Investigation.filter({
      objective: "60-Day Contractor Workflow — Persistent Self-Reflection & Autonomous Implementation Tracking",
    });

    let investigation;
    const investigationObjective = "60-Day Contractor Workflow — Persistent Self-Reflection & Autonomous Implementation Tracking";
    const investigationQuestions = WORKFLOW_STAGES.map((s) => `Stage ${s.id} (${s.name}): Is the tool for this stage fully implemented and operational? What remains?`);
    const investigationHypotheses = [
      "All 12 stages of the 60-day workflow can be autonomously implemented by Alpha Prime using existing backend functions and entity CRUD.",
      "Each workflow tool maps to an existing admin route — implementation is primarily UI wiring and backend function connection.",
      "Self-reflection on implementation status enables Alpha Prime to prioritize remaining work and report progress to the owner.",
    ];

    if (existingInvestigations && existingInvestigations.length > 0) {
      // Update existing investigation with latest reflection
      investigation = existingInvestigations[0];
      const newFinding = {
        finding: `Sync run at ${new Date().toISOString()}: ${createdCount} new workflow tasks created. Total tracked: ${(existingTasks?.length || 0) + createdCount} of 60 days. Alpha Prime is aware of all 12 stages and their implementation status.`,
        confidence: 0.95,
        category: "workflow_sync",
      };
      const updatedFindings = [...(investigation.findings || []), newFinding];
      const updatedAgents = [...new Set([...(investigation.agents_involved || []), "alpha_prime_orchestrator", "system"])];
      investigation = await base44.asServiceRole.entities.Investigation.update(investigation.id, {
        findings: updatedFindings,
        agents_involved: updatedAgents,
        status: "investigating",
        retrievals_performed: (investigation.retrievals_performed || 0) + 1,
      });
    } else {
      // Create new investigation
      investigation = await base44.asServiceRole.entities.Investigation.create({
        objective: investigationObjective,
        requester: "system",
        priority: "critical",
        target_type: "system",
        target_label: "60-Day Contractor Workflow Implementation",
        playbook: "alpha_prime_self_reflection",
        questions: investigationQuestions,
        hypotheses: investigationHypotheses,
        evidence: [
          {
            source: "workflow_data",
            content: `60-day workflow synced with ${WORKFLOW_DAYS.length} days across ${WORKFLOW_STAGES.length} stages. Each day maps to a specific tool and admin route.`,
            confidence: 1.0,
        ...
        findings: [
          {
            finding: `Initial sync: ${createdCount} workflow tasks created for Alpha Prime to track. Total: ${createdCount} of 60 days. Alpha Prime now has persistent awareness of all 12 stages.`,
            confidence: 0.95,
            category: "workflow_sync",
          },
        ],
        status: "investigating",
        agents_involved: ["alpha_prime_orchestrator", "system"],
        retrievals_performed: 1,
      });
    }

    // Step 4: Send a message to Alpha Prime in the swarm
    await base44.asServiceRole.entities.SwarmMessage.create({
      from_agent: "system",
      to_agent: "alpha_prime_orchestrator",
      message_type: "directive",
      content: `Workflow sync complete. ${createdCount} new tasks created. Total tracked: ${(existingTasks?.length || 0) + createdCount} of 60 days across 12 stages. You now have persistent awareness of the entire 60-day contractor workflow. Your self-reflection protocol is active: review each stage, assess implementation status, and autonomously implement tools where possible. Mark SwarmTasks as 'completed' when a tool goes live. Investigation ${investigation?.id || ""} is your persistent tracking record.`,
    });

    return Response.json({
      success: true,
      message: `Workflow synced to swarm. ${createdCount} new tasks created. ${(existingTasks?.length || 0) + createdCount} total tasks tracked.`,
      tasksCreated: createdCount,
      totalTracked: (existingTasks?.length || 0) + createdCount,
      investigationId: investigation?.id,
      stagesTracked: WORKFLOW_STAGES.length,
      totalDays: WORKFLOW_DAYS.length,
    });
  } catch (error) {
    console.error("syncWorkflowToSwarm error:", error);
    return Response.json({ error: error.message || "Swarm sync failed" }, { status: 500 });
  }
}