...
//   - Workers authenticate with a dedicated WORKER_TOKEN (not client credentials)
//   - Worker identity (worker_id) is recorded in every table it touches
//   - pgmq_message_id is tracked for full lineage
//   - No service credentials are stored in jobs or queue messages
// ════════════════════════════════════════════════════════════════
...
export interface WorkerConfig {
  ...
  supabase_url: string;
  worker_token: string;        // Dedicated worker auth token (NOT client credentials)
...
export interface JobEnvelope {
  ...
  pgmq_message_id?: number;   // Lineage: queue message -> job
}
...
export abstract class BaseWorker {
  ...
  protected current_pgmq_msg_id: number | null = null;
  ...
  constructor(config: WorkerConfig) {
    this.config = config;
    // Workers use their dedicated token, not client credentials
    this.supabase = createClient(config.supabase_url, config.worker_token);
  }
  ...
  async pollQueue(): Promise<JobEnvelope | null> {
    ...
    // Extract pgmq_message_id for lineage tracking
    const msg = data[0];
    const job = msg.message as JobEnvelope;
    job.pgmq_message_id = msg.msg_id;
    this.current_pgmq_msg_id = msg.msg_id;
    return job;
  }
  ...
  async claimAndProcess(job: JobEnvelope) {
    ...
    const { data, error } = await this.supabase.from('jobs').update({
      status: 'in_progress',
      claimed_by: this.config.worker_id,
  ...
  async completeJob(job: JobEnvelope, result: any) {
    ...
    // Record job attempt with full lineage: pgmq_message_id + worker_id
    ...
    if (job.pgmq_message_id || this.current_pgmq_msg_id) {
      await this.supabase.rpc('queue_archive', {
        p_queue_name: this.getQueueName(),
        p_msg_id: job.pgmq_message_id || this.current_pgmq_msg_id,
      });
    }
  ...
  async failJob(job: JobEnvelope, error: string) {
    ...
    // Record failed attempt with lineage
    await this.supabase.from('job_attempts').insert({
      job_id: job.job_id,
      worker_id: this.config.worker_id,
      pgmq_message_id: job.pgmq_message_id || this.current_pgmq_msg_id,
    ...
    if (newAttempt >= job.max_attempts) {
      ...
      await this.supabase.rpc('queue_send', {
        p_queue_name: 'dead_letter',
        p_msg: { ...job, error, failed_at: now },
      });
      await this.supabase.from('incidents').insert({
        incident_id: `inc-${job.job_id}-${Date.now()}`,
        organization_id: job.organization_id,
        system_id: job.system_id,
        severity: 'high',
        title: `Job ${job.job_id} exceeded max attempts`,
        description: error,
        status: 'open',
  ...
  async uploadArtifact(systemId: string, organizationId: string, artifactType: string, data: any): Promise<string> {
  ...
  // Evidence receipt now includes worker_id and pgmq_message_id for full lineage
  async writeEvidenceReceipt(
    systemId: string,
    ...
    await this.supabase.from('evidence_receipts').insert({
      ...
      verified_by: this.config.worker_id,
      worker_id: this.config.worker_id,
      pgmq_message_id: this.current_pgmq_msg_id,
      valid: true,
    });
    return receiptId;
  }
